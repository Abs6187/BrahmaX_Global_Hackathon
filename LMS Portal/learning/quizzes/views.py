...
def createAssignment(request):
	if request.method == 'POST':
		...
		CA.task = request.FILES.get('task')
		CA.save()
...
def assignstu(request):
	...
	enrolled_courses = Enroll.objects.filter(student=request.user).values_list('course', flat=True)
	
	# Get all assignments for these courses
	assignments = CreateAssignment.objects.filter(info__in=enrolled_courses)
	
	# Get submitted assignments
	submitted = SubmitAssignment.objects.filter(student=request.user)
	
	return render(request, 'assignstu.html', {
		'assign': submitted,
